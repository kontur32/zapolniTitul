...
function forms:main ( $page, $id, $datainst, $dataver, $message ) {
  ...
  let $userForms := 
        try {
          let $requestPath := "http://localhost:9984/zapolnititul/api/v2/users/" || session:get( "userid" ) || "/forms"
  ...
  let $currentFormID := 
    ...
    then( $id )
      ...
      let $userFormID :=
        try{
          fetch:xml( "http://localhost:9984/zapolnititul/api/v2/users/" || 
            session:get( "userid" ) || 
  ...
  return 
    ...
    then(
      ...
      return
        ...
        web:redirect( "http://localhost:9984/zapolnititul/forms/u/" )
    )
};