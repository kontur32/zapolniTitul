               ...
               <select class="form-control" name="{ $field/ID/text() }">
                 {
                  let $itemsQueryURL :=
                    ...
                    then (
                      "http://localhost:9984/zapolnititul/api/v1/forms/data/" 
                      || $id 
                      || "?f=" 
                      || $field/ID/text()
                    )
                  ...
                  let $items := 
                    let $dataSource := 
                      ...
                      try{
                       ...
                       try {
                          fetch:xml("http://localhost:9984/zapolnititul/api/v2/forms/"|| $id|| "/data")/data/table/row/cell[ @label = $field/ID/text() ]
                       }
                       catch*{
